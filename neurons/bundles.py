...
class LIFrate:
    ...
    def __init__(self,
        ...
        tau_s=10,        # synapse time constant
...
class adaptiveLIFrate:
    ...
    def __init__(self,
        ...
        self.current = np.zeros(shape=(self.size), dtype=np.float32)
        self.voltage = self.v_rest * np.ones(shape=(self.size), dtype=np.float32)
        self.adapt = np.zeros(shape=self.size, dtype=np.float32)
        self.outputs = np.zeros(shape=self.size, dtype=np.float32)
    ...
    def step(self, dt=0.001):
        ...
        dv = (self.v_rest - self.voltage - self.adapt + self.R * self.current) / self.tau_v * dt
        da = (-self.adapt + self.a * self.outputs) / self.tau_a * dt
...
class Decoder:
    ...
    def __init__(self, input_size, size, tau=1, activation=sigmoid):
        self.ntype = 'Decoder'
        ...
        self.premise = np.zeros(size, dtype=np.float32)
        self.outputs = np.zeros(size, dtype=np.float32)
        
        self.inputs = np.zeros(input_size, dtype=np.float32)
        self.decoders = np.random.normal(0, 0.1, size=(size, input_size)).astype(np.float32)
        
        self.train = False
        self._history = []
        
    def __repr__(self):
        return f'Decoder(size={self.size})'
    ...
    def step(self, dt=0.001):
        ...
        if self.train:
            self.decoders -= 2 / self.size * np.outer((self.outputs - self.premise) * self.activation(self.decoders @ self.inputs, deriv=True), self.inputs) / self.tau * dt
            
        self.outputs = self.activation(self.decoders @ self.inputs)
